...
def image_vuln_pubsub_handler(event, context):
    ...
    # Filter to process only VULNERABILITY occurrences
    print(f"Occurrence kind: {occurrence.kind}")
    
    if occurrence.kind != containeranalysis_v1.enums.NoteKind.VULNERABILITY:
        print(f"Skipping non-vulnerability occurrence: {occurrence_name}")
        return

    # Create a simple file name based on the occurrence ID
    occurrence_id = occurrence_name.split('/')[-1]  # Extract the occurrence ID
    destination_object_name = f"{occurrence_id}.jsob"  # Use the ID as the filename
