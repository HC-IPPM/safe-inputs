...
type Rule = (user: Express.User) => void;

const email_has_allowed_basic_host: Rule = (user) => {
...
const email_has_allowed_privileged_host: Rule = (user) => {
  const email = get_user_email(user);
...
const email_is_super_user: Rule = (user) => {
  const email = get_user_email(user);
...
const apply_rules_to_user = (user: Express.User, ...rules: Rule[]) =>
  rules.forEach((rule) => rule(user));

export const validate_user_email_allowed = (user: Express.User) =>
...
export const validate_user_can_have_privileges = (user: Express.User) =>
  apply_rules_to_user(
    user,
...
export const validate_user_is_super_user = (user: Express.User) =>
  apply_rules_to_user(
    user,
    email_has_allowed_basic_host,
    email_has_allowed_privileged_host,
